...
export const deleteComment = async (req, res, next) => {
    try {
        ...
        const dltComment = await Comment.findByIdAndDelete(req.params.commentId);
        res.status(200).json({ success: true, data: dltComment });
    } catch (error) {
        next(error)
    }
}
...
export const getComments = async (req, res, next) => {
    try {
        if (!req.user.isAdmin) {
            return next(errorHandler(403, "You are not allow to get all comment"))
        }
